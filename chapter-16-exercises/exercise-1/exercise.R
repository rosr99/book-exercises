...
ggplot(diamonds_sample) + geom_blank() # I see nothing and I expected nothing.
...
ggplot(diamonds_sample, aes(x = carat, y = price)) + geom_point(color = "blue")

# Draw a scatter plot for `diamonds_sample` of `price` by `carat`, where each
# point has an aesthetic _shape_ based on the diamond's `cut`.

ggplot(diamonds_sample, aes(x = carat, y = price, shape = cut)) + geom_point()

# Draw a scatter plot for `diamonds_sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*

ggplot(diamonds_sample, aes(x = carat, y = cut, size = price)) + geom_point()

# Try coloring the above plot based on the diamond's price!

ggplot(diamonds_sample, aes(x = carat, y = cut, size = price, color = price)) + geom_point()

# Draw a line plot (with line geometry) for `diamonds_sample`. The x-position 
# should be mapped to carat, y-position to price, and color to cut.

ggplot(diamonds_sample, aes(x = carat, y = price, color = cut)) + geom_line()

# That's kind of messy. Try using `smooth` geometry instead.

ggplot(diamonds_sample, aes(x = carat, y = price, color = cut)) + geom_smooth()
...
ggplot(diamonds_sample, aes(x = cut, y = price)) + geom_col()

# Add an aesthetic property that will _fill_ each bar geometry based on the 
# `clarity` of the diamonds. 
# What kind of chart do you get?

ggplot(diamonds_sample, aes(x = cut, y = price, fill = clarity)) + geom_col() #bar graph
...
ggplot(diamonds_sample, aes(x = carat, y = price, color = carat)) + geom_point() + geom_smooth()
